...
def initialize() -> None:
    ...
    """
    global training_data, test_data, c_vector, tfidf_transformer
...
def train_categories(train_data) -> SGDClassifier:
    """
    Trains model using textual data.
    """
    training_counts = c_vector.fit_transform(train_data[0])
...
def predict_categories() -> None:
    ...
    test_counts = c_vector.transform(test_data)

    # Term frequency and term frequency times inverse document frequency transformations
    transformed_test = tfidf_transformer.transform(test_counts)
...
def predict_subject_category(feedback_entry: str, classifier: SGDClassifier) -> str:
    """
    Predicts the subject category for a text.
    """
    training_counts = c_vector.transform([feedback_entry])

    # Term frequency and term frequency times inverse document frequency transformations
    transformed_test = tfidf_transformer.transform(training_counts)
    predicted = classifier.predict(transformed_test)
    return config.review_type[predicted[0]]
...
def predict_employer() -> None:
    ...
    test_counts = c_vector.transform(config.employer_test[0])
